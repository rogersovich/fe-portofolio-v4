      ...
      <div class="flex justify-between">
        <div>
          ...
          <div class="text-muted-foreground font-light mt-2">
            Create reliable Experience in here
          </div>
        </div>
        <div>
        ...
        <div class="col-span-3">
          <div class="flex flex-col gap-1 text-left">
            ...
            <div class="flex flex-col items-center justify-center gap-3 mb-3">
              <template v-if="forms?.comp_image?.file">
                <NuxtImg
                  :src="forms?.comp_image.blob_url"
                  height="100"
                  class="rounded-lg"
                  fit="contain"
                ></NuxtImg>
...
const handleImageChange = (event: any) => {
  ...
  if (file) {
    forms.value.comp_image.blob_url = URL.createObjectURL(file);
    forms.value.comp_image.file = file;
  }
};
...
const cancelImage = () => {
  forms.value.comp_image.file = null;
...
const validateImage = () => {
  imageError.value = "";
  if (!forms.value.comp_image.file) {
...
// Update
const { storeExperience, loading } = useExperienceAPI();
...
const onFormSubmit = async () => {
  ...
  if (!summaryHtmlError.value && isValid && isValidImage) {
    ...
    const avatarNewFile = forms.value.comp_image.file as unknown as File;
    formData.append("comp_image_file", avatarNewFile);

    await storeExperience(formData);
  }
};
</script>
<style lang=""></style>
